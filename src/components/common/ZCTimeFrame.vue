  ...
  <div class="IM_time_frame bgc_rzl" id="IM_time_frame"
    ...
    <div class="IM_time_frame_content clearfix">
      ...
      <!--<div class="IM_time_frame_report bgc_base"-->
           <!--id="getPdfByDom"-->
           <!--@click="beginToScreenShot">-->
        <!--<span id="report-span">{{canGetPdf?'生成报告':'报告生成中'}}</span>-->
      <!--</div>-->


      <div class="IM_time_frame_report bgc_base" id="getPdfByDom">

        <el-dropdown trigger="click" @command="generationReport">
          <span style="color: #ffffff" id="report-span">
            {{canGetPdf?'生成报告':'报告生成中'}}
          </span>
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item command="1">生成PDF报告</el-dropdown-item>
            <el-dropdown-item command="2">生成WORD报告</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </div>
    ...
    methods: {
      ...
      handleDateData(dateM) {
        ...
        if (this.selectedPickerType.name === '周') {
          let day = date.getDay();
          let result = date.setDate(date.getDate()- (day>0?day:7) + 1);
      ...
      /************生成报告方法*****************/
      generationReport(type){

        if(type == 1){
          this.getPdfByDom();
        }else if(type == 2){
          this.beginToScreenShot()
        }
      },
