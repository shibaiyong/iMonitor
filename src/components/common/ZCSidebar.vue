<template>
  <div id="IM_sidebar" class="IM_sidebar bgc_rzl" :class="options.showSidebar?'':'IM_sidebar_hidden'">
    ...
    <div class="IM_sidebar_tools">
      ...
      <div class="IM_sidebar_tools_item" @click="goToCustomization">+ 定制监测项</div>
  ...
  export default {
    ...
    methods: {
      ...
      iMSidebarClick(type, clickIndex, event) {
        ...
        let isHistory = true;
        if (event && event.currentTarget.className === 'dialog_title') {
          isHistory = false;
        }

        let indexArr = clickIndex, typeID, typeName, typeState, thiz = this;
        ...
        this.sidebarData_common.forEach(function (commonValue, commonIndex) {
          ...
          if (commonValue.history) {
            ...
            commonValue.history.forEach(function (historyItem, historyIndex) {
              ...
              if (type === 0 && indexArr.length === 2
                && !thiz.showDialog
                && historyIndex === indexArr[1]
                && commonIndex === indexArr[0]
                && isHistory) {
          ...
          if (commonValue.list) {
            ...
            commonValue.list.forEach(function (listItem, listIndex) {
              ...
              if (type === 0 && indexArr.length === 2 && listIndex === indexArr[1] && commonIndex === indexArr[0] && !isHistory) {
              ...
              } else if (type === 0 && indexArr.length === 2 && listItem.id === clickHistoryId  && isHistory) {
                listItem.selected = true;
              }
              else {
                //当点击的不是单个平台,取消选中
      ...
      goToCustomization() {
        ...
        // this.$router.push('/customconfigure');
      },
      // 跳转
      goToAddArticle() {
